...
class BaseModel(nn.Module):
    ...
    def _initialize_models(self, args, device):
        ...
        self.real_score = WanDiffusionWrapper(
            model_name=self.real_model_name, load_dtype=torch.bfloat16, is_causal=False)
        self.real_score.model.requires_grad_(False)

        self.fake_score = WanDiffusionWrapper(
            model_name=self.fake_model_name, load_dtype=torch.float32, is_causal=False)
...
class SelfForcingModel(BaseModel):
    ...
    def _run_generator(
        ...
        min_num_frames = getattr(self.args, "min_training_frames", 21)
        max_num_frames = getattr(self.args, "max_training_frames", self.num_training_frames)
