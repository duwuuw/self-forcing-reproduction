...
class Trainer:
    ...
    def _checkpoint_path(self):
        return Path(self.config.resume) / "model.pt"

    def _load_resume(self):
        if not getattr(self.config, "resume", ""):
            return
        ...
        if self.generator_ema is not None and checkpoint.get("generator_ema") is not None:
            self.generator_ema.load_state_dict(checkpoint["generator_ema"])
        print(f"Resumed checkpoint={path} step={self.step}", flush=True)
    ...
    def train(self, max_steps=None, grad_accum_steps=1):
        ...
        while max_steps is None or self.step < max_steps:
            ...
            if TRAIN_GENERATOR:
                ...
                for _ in range(grad_accum_steps):
                    ...
                    extra = self.fwdbwd_one_step(batch, True, 1.0 / grad_accum_steps)
            ...
            for _ in range(grad_accum_steps):
                batch = next(self.dataloader)
                extra = self.fwdbwd_one_step(batch, False, 1.0 / grad_accum_steps)
                extras_list.append(extra)
            ...
            if self.is_main_process:
                ...
                print(
                    f"step={self.step} "
                    f"generator_loss={wandb_loss_dict.get('generator_loss', 'NA')} "
                    f"critic_loss={wandb_loss_dict['critic_loss']}",
                    flush=True,
                )
